...
class ConvPre(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, bias=None, stride=1, padding=(1,1), dilation=1, groups=1):
        super(ConvPre, self).__init__()
        kernel = torch.randn(out_channel,1,kernel_size,kernel_size)
    ...
    def forward(self, x):
        _conv2d = lambda x: F.conv2d(x, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
        y = torch.cat(
            [_conv2d(x[:, idx].unsqueeze(1)) for idx in range(x.shape[1])],
            dim=1)
        return y
...
class ConvNext(nn.Module):
    ...
    def forward(self, x):
        features = torch.stack(x.split(self.in_channel, dim=1))
        y = torch.cat(
            [feature.sum(dim=1, keepdim=True) for feature in features],
            dim=1)
        return y

class BasicConv2d(nn.Module):

    def __init__(self, input_channels, output_channels, **kwargs,flag=False):
        super().__init__()
        if flag:
            self.conv=nn.squential(
                ConvPre(input_channels, output_channels, bias=False, **kwargs),
                ConvNext(input_channels, output_channels, bias=False)
            )
        else:
...
class InceptionE(nn.Module):
    def __init__(self, input_channels,flag=False):
        super().__init__()
        ...
        if flag:
            self.branch3x3stack_3b = BasicConv2d(384, 384, kernel_size=(3, 1), padding=(1, 0),flag=True)
        else:
...
class InceptionV3(nn.Module):
    ...
    def __init__(self, in_channels=3, num_classes=10):
        ...
        self.Mixed_7c = InceptionE(2048,flag=True)
